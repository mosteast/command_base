...
function decode_basic_html_entities(input_text) {
  return input_text.replace(
    /&(nbsp|amp|lt|gt|quot|apos|#34|#39|#x27);/gi,
    (match) => {
      ...
      if (
        Object.prototype.hasOwnProperty.call(html_entity_map, normalized_key)
      ) {
...
function strip_markdown_fences(source_text) {
  return source_text.replace(
    /```(.*?)(\n)?([\s\S]*?)```/g,
    (match, info_string, _line_break, code_body) => {
...
function convert_markdown_to_plain_text(markdown_input) {
  ...
  normalized_text = normalized_text.replace(
    /`([^`]+)`/g,
    (_match, code_text) => code_text,
  );

  normalized_text = normalized_text.replace(
    /\!\[([^\]]*)\]\(([^)]+)\)/g,
    (_match, alt_text) => alt_text || "",
  );

  normalized_text = normalized_text.replace(
    /\[([^\]]+)\]\(([^)]+)\)/g,
    (_match, link_label, link_target) => {
  ...
  normalized_text = normalized_text.replace(
    /^[ \t]*(#{1,6})[ \t]+(.+)$/gm,
    (_match, _hashes, heading_text) => heading_text.trim(),
  );

  normalized_text = normalized_text.replace(
    /^(.*)\n={3,}\s*$/gm,
    (match, heading_text) => heading_text.trim(),
  );
  normalized_text = normalized_text.replace(
    /^(.*)\n-{3,}\s*$/gm,
    (match, heading_text) => heading_text.trim(),
