...
async function cleanup_interrupted_transcript_outputs(
  output_key_path,
  options = {},
) {
  const {
...
function resolve_transcript_outputs(options = {}) {
  const { inputPath, outputDir, outputFormats, outputFormat, outputBaseName } =
    options;
...
async function run_python_runner(options = {}) {
  ...
  try {
    ...
    for (const format of requested_formats) {
      ...
      await clear_hidden_flag_if_needed(final_candidate, { logger, debug });
