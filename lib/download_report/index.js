...
const DEFAULT_REPORT_ROOT_PARTS = [
  "Library",
  "Mobile Documents",
  "com~apple~CloudDocs",
  "main",
...
function create_default_report_root(
  home_dir = process.env.HOME || process.cwd(),
) {
  return path.join(
    String(home_dir || process.cwd()),
    ...DEFAULT_REPORT_ROOT_PARTS,
  );
}

function read_directory_entries(directory_path) {
  try {
    return fs_sync.readdirSync(directory_path, { withFileTypes: true });
  } catch (error) {
    if (error && error.code === "ENOENT") return [];
    throw error;
  }
...
function normalize_report_batch_number(value) {
  const text = String(value || "").trim();
  if (!/^[1-9]\d*$/.test(text)) {
    throw new Error(`report batch number must be a positive integer: ${value}`);
  }
  return Number(text);
}

function get_largest_report_batch_number(options = {}) {
  const report_root =
    options.report_root || create_default_report_root(options.home_dir);
  let largest_batch_number = 0;

  for (const tool_entry of read_directory_entries(report_root)) {
    if (!tool_entry.isDirectory()) continue;
    const tool_dir = path.join(report_root, tool_entry.name);
    for (const batch_entry of read_directory_entries(tool_dir)) {
      if (!batch_entry.isDirectory()) continue;
      if (!/^[1-9]\d*$/.test(batch_entry.name)) continue;
      largest_batch_number = Math.max(
        largest_batch_number,
        Number(batch_entry.name),
      );
    }
  }
...
function get_next_report_batch_number(options = {}) {
  return get_largest_report_batch_number(options) + 1;
}

function create_report_dir_for_batch(tool_name, batch_number, options = {}) {
  const report_root =
    options.report_root || create_default_report_root(options.home_dir);
  const normalized_batch_number = normalize_report_batch_number(batch_number);
  return path.join(
    report_root,
    sanitize_report_path_component(tool_name),
    String(normalized_batch_number),
  );
}

function create_default_report_dir(tool_name, options = {}) {
  const batch_number =
    options.batch_number || get_next_report_batch_number(options);
  return create_report_dir_for_batch(tool_name, batch_number, options);
}
...
module.exports = {
  ...
  create_report_dir_for_batch,
  get_largest_report_batch_number,
  get_next_report_batch_number,
