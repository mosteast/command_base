...
const { classify_gallery_dl_probe } = require("../probe_classify");
const {
  get_cookies_file,
...
async function check(context) {
  ...
  const gallery = await brew_install.check_gallery_dl(context.tool_options || {});
  checks.push({
    name: "gallery-dl",
  ...
  const cookie_file_info = await cookie_export.cookies_file_has_hosts(
    runtime_cookies,
    "x_gallery_dl",
  );
  checks.push({
  ...
  if (!context.offline && gallery.ok) {
    ...
    const args = ["--simulate", "--no-input", "--range", "1", "--post-range", "1"];
    if (cookie_file_info.exists) {
      args.push("--cookies", runtime_cookies);
    }
    args.push(probe_url);
    const probe = await cookie_export.run_command("gallery-dl", args, {
      timeout_ms: 30000,
    });
    const classified = classify_gallery_dl_probe(
      `${probe.stdout}\n${probe.stderr}`,
      { ok: probe.ok, timed_out: probe.timed_out },
    );
