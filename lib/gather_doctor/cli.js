...
const { normalize_platform_list, select_platforms } = require("./platform");
...
function build_help_text(script_name) {
  return [
    ...
    "                               fix still writes that mapping when check is already ok",
    ...
    "",
    "  # Map instagram to a named Chrome profile",
    "  $0 fix --platform instagram --chrome-profile Default --yes",
  ].join("\n");
}
...
function parse_arguments(argv) {
  ...
  const parser = yargs(args)
    ...
    .option("chrome-profile", {
      type: "string",
      describe: "Limit Chrome scan to one profile directory or name",
    })
    .option("offline", {
      type: "boolean",
      default: false,
  ...
  return {
    ...
    runtime_path: parsed.runtime
      ? String(parsed.runtime)
      : DEFAULT_RUNTIME_PATH,
