...
function run_command(command, args, options = {}) {
  return new Promise((resolve) => {
    ...
    let timed_out = false;
    let timeout_id = null;
    let kill_timeout_id = null;
    const timeout_ms = Number(options.timeout_ms) || 0;

    const finish = (result) => {
      if (timeout_id) clearTimeout(timeout_id);
      if (kill_timeout_id) clearTimeout(kill_timeout_id);
      resolve(result);
    };
    ...
    child.on("close", (code) => {
      finish({
        ok: code === 0 && !timed_out,
        exit_code: timed_out ? 124 : typeof code === "number" ? code : 1,
        timed_out,
        stdout,
        stderr: timed_out
          ? `${stderr}\ncommand timed out after ${timeout_ms}ms`.trim()
          : stderr,
      });
    });
  });
