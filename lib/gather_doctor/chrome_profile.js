...
async function read_cookie_hosts(cookies_path, options = {}) {
  ...
  const copy_path = `${cookies_path}.gather_doctor_copy`;
  let used_copy = false;
...
function match_chrome_profile(matches, query) {
  ...
  for (const item of matches || []) {
    ...
    const combo = `${directory}/${name}`;
    if (directory === wanted || name === wanted || combo === wanted)
      found.push(item);
  }
