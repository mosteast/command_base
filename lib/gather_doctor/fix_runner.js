...
const { match_chrome_profile } = require("./chrome_profile");
...
function should_plan_fix(result, options = {}) {
  if (options.force) return true;
  if (options.chrome_profile) return true;
  if (result.status !== "ok") return true;
  return Boolean(result.profile_matches && result.profile_matches.length > 1);
}

function apply_profile_query(results, query) {
  const wanted = String(query || "").trim();
  if (!wanted) return { ok: true, results };
  let matched_any = false;
  const next_results = (results || []).map((result) => {
    const matched = match_chrome_profile(result.profile_matches, wanted);
    if (!matched) return result;
    matched_any = true;
    return { ...result, selected_profile: matched };
  });
  return { ok: matched_any, results: next_results };
}
...
async function run_fix(options) {
  ...
  let planned_platforms = [];

  for (const result of diagnosis.results) {
    if (!should_plan_fix(result, options)) {
    ...
    if (result.profile_matches && result.profile_matches.length > 1) {
      plan_lines.push(
        `- ${result.platform_key}: choose Chrome profile (candidates: ${result.profile_matches
          ...
          .join(", ")}); type a name, or y/--yes for most recently used`,
      );
  ...
  const confirmation = await confirm_plan(plan_lines, {
    yes: options.yes,
    dry_run: false,
    skip_plan: true,
  });
  ...
  if (confirmation.profile_query) {
    const applied = apply_profile_query(
      planned_platforms,
      confirmation.profile_query,
    );
    if (!applied.ok) {
      logger.warn(
        `Unknown Chrome profile "${confirmation.profile_query}".`,
      );
      logger.warn("Fix cancelled.");
      return { exit_code: 1, diagnosis, cancelled: true };
    }
    planned_platforms = applied.results;
  }
