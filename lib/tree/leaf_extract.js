...
const DEFAULT_CHILDREN_CANDIDATES = [
  "r_children",
  "children",
  "nodes",
  "items",
...
function get_leaf_nodes(root_node) {
  ...
  return root_node.all({ strategy: "post" }, (node) => {
    if (
      node.model &&
      node.model.payload &&
      node.model.payload.__leaf_extract_root
    ) {
      return false;
    }
...
function collect_output_nodes(current_node, selected_nodes, options) {
  ...
  for (const child_node of child_nodes) {
    const child_outputs = collect_output_nodes(
      child_node,
      selected_nodes,
      options,
    );
...
function extract_leaf_levels(input_data, options = {}) {
  ...
  for (const child_node of root_node.children || []) {
    output_forest.push(
      ...collect_output_nodes(child_node, selected_nodes, normalized_options),
    );
  }
...
function collect_nodes_to_level(root_node, level) {
  const selected_nodes = new Set();
...
function extract_tree_levels(input_data, options = {}) {
  const parsed_level = Number(options.level);
  const level =
    Number.isFinite(parsed_level) && parsed_level >= 1
      ? Math.floor(parsed_level)
  ...
  const selected_nodes = collect_nodes_to_level(root_node, level);
  const output_forest = [];
  ...
  function map_nodes(node_list) {
    ...
    for (const node of node_list) {
      ...
      const mapped_is_object =
        mapped_payload &&
        typeof mapped_payload === "object" &&
        !Array.isArray(mapped_payload);
      ...
      if (!mapped_is_object) {
        ...
        const wrapped_payload = {
          [normalized_options.value_key]: mapped_payload,
        };
