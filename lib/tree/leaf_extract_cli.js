...
const { extract_leaf_levels, extract_tree_levels } = require("./leaf_extract");
...
function build_help_text(config) {
  ...
  return [
    ...
    `  Extract ${config.format_label} tree slices by level.`,
    "  Default mode is from root (--from-leaf disabled):",
    "  level 1 keeps top-level nodes, level 2 keeps two levels, etc.",
    "  Enable --from-leaf for legacy leaf-path extraction.",
    "",
    `${chalk.bold("Options")}`,
    "  -l, --level <n>        Tree depth to extract (default: 1)",
    "  --from-leaf            Extract from leaf paths (legacy behavior, default: false)",
    "  --children-key <key>   Children key in tree nodes (default: auto-detect r_children/children)",
    ...
    "                         Output name: <base>.(tree|leaves).<level><ext>",
    ...
    "",
    `${chalk.bold("Examples")}`,
    `  # Extract root level 1 from every tree (default mode)`,
    `  ${script_name} "${sample_glob}"`,
    "",
    `  # Extract level 2 from root into the output directory`,
    `  ${script_name} --level 2 --out output "${sample_glob}"`,
    "",
    `  # Keep previous leaf-based behavior`,
    `  ${script_name} --from-leaf --level 1 "${sample_glob}"`,
    "",
...
function parse_cli_arguments(config) {
  const parser = yargs(process.argv.slice(2))
    ...
    .option("level", {
      ...
      type: "number",
      default: 1,
      describe: "Tree depth to extract",
    })
    .option("from-leaf", {
      type: "boolean",
      default: false,
      describe: "Extract from leaf paths (legacy behavior)",
    })
    .option("children-key", {
      type: "string",
  ...
  return {
    patterns,
    level,
    from_leaf: Boolean(argv["from-leaf"]),
...
function build_output_extension_with_level(output_extension, level, from_leaf) {
  const safe_extension = output_extension.startsWith(".")
    ? output_extension
    : `.${output_extension}`;
  const mode_text = from_leaf ? "leaves" : "tree";
  return `.${mode_text}.${level}${safe_extension}`;
}

function build_output_path_with_level(
  input_path,
  output_directory,
  output_extension,
  level,
  ...
  const level_suffix = build_output_extension_with_level(
    output_extension,
    level,
    from_leaf,
  );
...
function is_output_file(input_path, output_extension, refresh_mode) {
  ...
  const safe_extension = output_extension.startsWith(".")
    ? output_extension
    : `.${output_extension}`;
  const escaped_extension = safe_extension.replace(
    /[.*+?^${}()|[\\]\\\\]/g,
    "\\\\$&",
  );
  const output_regex = new RegExp(
    `\\\\.(leaves|tree)\\\\.\\\\d+${escaped_extension}$`,
  );
...
async function convert_stdin(options, config) {
  ...
  try {
    ...
    const extract_fn = options.from_leaf
      ? extract_leaf_levels
      : extract_tree_levels;
    logger.debug(
      `Extracting tree levels (mode=${options.from_leaf ? "from_leaf" : "from_root"}).`,
    );
    const { output_data } = extract_fn(parsed_data, {
      level: options.level,
...
async function convert_files(options, config, logger) {
  ...
  for (const input_path of input_files) {
    ...
    if (
      is_output_file(input_path, options.output_extension, options.refresh_mode)
    ) {
    ...
    try {
      ...
      const { output_data, meta } = extract_fn(parsed_data, {
        level: options.level,
        children_key: options.children_key,
        value_key: options.value_key,
      });

      logger.debug(`Tree extraction meta: ${JSON.stringify(meta)}`);
      ...
      if (options.dry_run) {
        ...
        await writeFile(
          output_path,
          ensure_trailing_newline(output_text),
          "utf8",
        );
        logger.info(
