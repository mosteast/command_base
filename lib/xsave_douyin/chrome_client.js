...
async function collect_list({
  ...
  let idle_scrolls = 0;
  while (last && last.has_more && (!limit || total < limit)) {
...
function should_skip_profile_path(file_path) {
  return /\/(Cache|Code Cache|GPUCache|OptimizationGuide)(\/|$)/.test(
    file_path,
  );
}

function copy_chrome_profile(source_dir) {
  const dest = fs.mkdtempSync(path.join(os.tmpdir(), "xsave-dy-profile-"));
  fs.cpSync(source_dir, dest, {
    recursive: true,
    dereference: true,
...
async function open_persistent_session({
  ...
  const copied = copy_chrome_profile(source_dir);
...
async function prepare_list_page(page, { mode, url, sec_user_id, intercepted_pages } = {}) {
  if (mode === "like") {
    const target = await resolve_user_profile_url(url, sec_user_id);
    if (!target) return;
    const like_wait = start_like_response_wait(page);
