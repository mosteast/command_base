...
function default_output_dir(mode, f2_root) {
  return path.join(
...
function resolve_output_dir(options = {}, deps = {}) {
  const explicit = String((options && options.path) || "").trim();
...
async function run_export(options, deps = {}) {
  ...
  const items_out = [];
  const item_limit = Number(process.env.COMMAND_BASE_F2_LIKE_LIMIT) || 0;
  ...
  for (const line of describe_export_layout({
    ...
    check_all: options && options.check_all,
    item_limit,
  }))
    logger.debug(line);
  ...
  try {
    ...
    const should_stop_list =
      options.check_all || options.mode === "one"
        ? undefined
        : async (items) => {
