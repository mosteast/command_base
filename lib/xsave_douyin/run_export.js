...
const { remove_empty_files } = require("../file/remove_empty_file");
...
async function cleanup_empty_files_before_collect(output_dir, options, logger) {
  if (options && options.dry_run) return;
  logger.debug(`Step: pre-export empty-file cleanup ${output_dir}`);
  const removed_count = await remove_empty_files(output_dir, logger);
  if (removed_count > 0) {
    logger.log(
      `Removed ${removed_count} empty file(s) from ${output_dir} before export.`,
    );
    return;
  }
  logger.debug(`No empty files found in ${output_dir} before export.`);
}
...
async function run_export(options, deps = {}) {
  ...
  await cleanup_empty_files_before_collect(output_dir, options, logger);
