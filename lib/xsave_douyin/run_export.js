...
function doctor_hint(chrome_profile) {
  return `gather doctor fix --platform douyin --chrome-profile ${
    chrome_profile || "nori"
  }`;
}
...
async function run_export(options, deps = {}) {
  ...
  const chrome_profile = (options && options.chrome_profile) || "nori";
  ...
  if (needs_session && !page) {
    ...
    try {
      ...
      page = session.page;
      if (resolved.prepare_list_page && options.mode !== "one") {
      ...
      try {
        ...
        if (planned.download) {
          ...
          if (!downloaded || !downloaded.ok) {
            logger.warn(
              `skip ${id} (${(downloaded && downloaded.reason) || "download_failed"})`,
            );
            continue;
          }
        }
