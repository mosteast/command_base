...
async function run_setup(options) {
  const logger = create_logger(options);
  logger.debug("Step: diagnose before setup");
  const diagnosis = await run_check({ ...options, offline: options.offline });

  const plan_lines = [
    chalk.cyanBright("Setup plan:"),
  ];
  ...
  if (planned_platforms.length === 0) {
    logger.success("Nothing to set up; all selected platforms are ok.");
  ...
  for (const result of planned_platforms) {
    logger.debug(`Step: setup ${result.platform_key}`);
    ...
    const setup_result = await adapter.setup(context, result);
    for (const action of setup_result.actions || []) {
    ...
    if (setup_result.runtime_patch && Object.keys(setup_result.runtime_patch).length > 0) {
      runtime_data = set_platform_runtime(
        runtime_data,
        result.platform_key,
        setup_result.runtime_patch,
      );
    }
  }
  ...
  logger.info("Re-running check after setup...");
