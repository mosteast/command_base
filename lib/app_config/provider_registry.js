...
const { APP_KEY: CURSOR_KEY, backup, restore } = require("./provider/cursor_provider");

const REGISTRY = {
  [CURSOR_KEY]: { backup, restore, label: "cursor" },
};
...
function get_provider(app_name) {
  const key = String(app_name || "").trim().toLowerCase();
