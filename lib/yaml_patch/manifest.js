...
function source_binding_for_participant(
  request_file,
  result,
  ...
  if (result_file.path !== request_file.path) {
    manifest_error("Manifest participant path binding is inconsistent", {
      file_id: request_file.id,
  ...
  for (const operation of proof.operations || []) {
    if (!request_operation_ids.has(operation.operation_id)) {
      manifest_error("Manifest byte proof names an undeclared operation", {
        file_id: request_file.id,
  ...
  if (result_file.diff !== undefined) {
    ...
    const diff_operation_ids = new Set(
      diff.operations
        .map((operation) => operation && operation.id)
        .filter((operation_id) => typeof operation_id === "string"),
    );
    for (const operation of proof.operations || []) {
      if (!diff_operation_ids.has(operation.operation_id)) {
        manifest_error("Manifest byte proof conflicts with diff operations", {
          file_id: request_file.id,
          operation_id: operation.operation_id,
        });
      }
...
function validate_manifest_participant_bindings(request, result) {
  if (
    ...
    manifest_error("Manifest request and result require operation/file arrays");
  }
  const request_operation_ids = new Set();
  for (const operation of request.operations) {
    if (
      ...
      request_operation_ids.has(operation.id)
    ) {
      manifest_error("Manifest request operation IDs must be unique");
    }
    request_operation_ids.add(operation.id);
  }
  ...
  const bindings = new Map(
    request.files.map((request_file) => [
      request_file.id,
      source_binding_for_participant(
        request_file,
        result,
        request_operation_ids,
      ),
