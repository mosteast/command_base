...
const RAW_REGEX_WORKER_CAPABILITY = Symbol("raw_regex_worker_capability");
...
function query_uses_raw_regex(query) {
  const pending = [query && query.where];
  while (pending.length > 0) {
    const predicate = pending.pop();
    if (!predicate || typeof predicate !== "object") continue;
    if (predicate.predicate === "raw_regex") return true;
    if (Array.isArray(predicate.all)) pending.push(...predicate.all);
    if (Array.isArray(predicate.any)) pending.push(...predicate.any);
    if (predicate.not) pending.push(predicate.not);
    if (predicate.predicate === "relation" && predicate.where) {
      pending.push(predicate.where);
    }
  }
  return false;
}

function raw_regex_capability_error() {
  throw new Yaml_patch_error(
    "CHANGE_LIMIT_EXCEEDED",
    "raw_regex execution requires an isolated worker",
    {
      details: {
...
function normalize_input_set(input_set) {
  ...
  return input_set.map((input, input_position) => {
    ...
    const source_path = path.resolve(requested_path);
    if (source_paths.has(source_path)) {
      request_error("Query input set contains a duplicate normalized path", {
        source_path,
        input_position,
      });
    }
...
function expectation_diagnostics(selected, query) {
  ...
  const max_candidates = Math.min(10, query.limits.max_result);
  for (const record of selected.slice(0, max_candidates)) {
...
function build_page(selected, offset, query, cursor_binding) {
  ...
  for (let index = 0; index < target_count; index += 1) {
    const record = selected[offset + index];
    const user_projection = project_query_results(
      [record],
      query.projection,
    )[0];
    const projection =
      cursor_binding.purpose === "candidate"
        ? candidate_diagnostic(record, user_projection)
        : user_projection;
...
function run_query_v2(input_set, query, options = {}) {
  const cloned_query = clone_query(query);
  if (
    query_uses_raw_regex(cloned_query) &&
    options[RAW_REGEX_WORKER_CAPABILITY] !== true
  ) {
    raw_regex_capability_error();
  }
  const validated_query = validate_query_v2(cloned_query, options);
  const inputs = normalize_input_set(input_set);
  const input_digest = digest_input_set(inputs);
  const semantic_query_digest = digest_query(validated_query);
  let cursor_purpose = "page";
  const cursor_query_digest = semantic_query_digest;
  let offset = 0;
  ...
  if (cursor_purpose !== "candidate") {
    const expectation_cursor = diagnostics.truncated
      ? create_query_cursor({
          purpose: "candidate",
          input_digest,
          query_digest: semantic_query_digest,
...
function run_query_v2_in_isolated_worker(input_set, query, options = {}) {
  if (isMainThread) raw_regex_capability_error();
  return run_query_v2(input_set, query, {
    ...options,
    [RAW_REGEX_WORKER_CAPABILITY]: true,
  });
}
...
module.exports = {
  ...
  run_query_v2,
  run_query_v2_in_isolated_worker,
