...
function normalize_validation_scope(scope = { kind: "all_inputs" }) {
  ...
  if (
    ["specified_files", "changed_files", "reference_closure"].includes(
      scope.kind,
    ) &&
    !Array.isArray(scope.files)
  ) {
    validation_error(`${scope.kind} scope requires files`);
  }
  ...
  for (const [field, values] of [
    ["files", scope.files],
    ["node_locators", scope.node_locators],
  ]) {
    if (values === undefined) continue;
    if (
      values.some((value) => typeof value !== "string" || value.length === 0)
    ) {
      validation_error(`validation scope ${field} must contain strings`);
    }
  }
