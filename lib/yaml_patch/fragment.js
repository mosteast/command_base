...
function edit_package_lock_path_for(output_directory) {
  ...
  return path.join(
    ...
    `.${path.basename(resolved_directory)}.yamlpatch-extract.lock`,
  );
}
...
async function write_edit_package(
  ...
  const temporary_directory = path.join(
    parent_directory,
    `.${output_name}.yamlpatch-session-${crypto.randomUUID()}`,
  );
  ...
  try {
    ...
    if (exists) {
      backup_directory = path.join(
        parent_directory,
        `.${output_name}.yamlpatch-backup-${crypto.randomUUID()}`,
      );
...
function validate_manifest(manifest) {
  ...
  if (
    manifest.format !== "yamlpatch-edit" ||
    manifest.version !== 1 ||
    !manifest.source ||
    !manifest.target ||
    ...
    throw new Yaml_patch_error(
      "INVALID_FRAGMENT",
      "manifest.json is not a supported yamlpatch edit manifest",
    );
  }
  assert_known_fields(
    manifest.source,
