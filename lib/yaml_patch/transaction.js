...
const { compile_operation } = require("./operation");
...
function assert_plain_known(value, allowed, label) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw_request_error(`${label} must be an object`);
  }
  for (const field of Object.keys(value)) {
    if (!allowed.includes(field))
      throw_request_error(`Unknown ${label} field: ${field}`);
  }
  return value;
}
...
function participant_digest_for(files) {
  ...
  return canonical_digest(
    files
      .map((file) => ({ id: file.id, digest: file.digest }))
      .sort((left, right) =>
...
function validate_participants(request) {
  if (!request || typeof request !== "object" || Array.isArray(request)) {
    throw_request_error("Transaction request must be an object");
  }
  ...
  for (const [index, file] of request.files.entries()) {
    if (
      ...
      throw_request_error(`Transaction file ${index} requires a non-empty id`);
    }
    assert_plain_known(file, FILE_FIELD, `transaction file ${index}`);
    if (typeof file.path !== "string" || file.path.length === 0) {
      throw_request_error(
        `Transaction file ${index} requires a non-empty path`,
      );
    }
  ...
  for (const [index, operation] of request.operations.entries()) {
    if (
      ...
      throw_request_error(`Transaction operation ${index} must be an object`);
    }
    if (
...
function index_for(file, buffer) {
...
function apply_compiled(state, compiled, operation_order) {
  ...
  state.index = index_for(state.file, state.buffer);
}
...
async function plan_transaction(request, options = {}) {
  ...
  for (const [operation_order, operation] of request.operations.entries()) {
    ...
    if (operation.type === "add_subtree") {
      const destination = resolve_reference(
        {
          file: operation.destination && operation.destination.file,
          selector: operation.destination && operation.destination.selector,
        },
        states,
        handles,
    ...
    if (operation.type === "delete_subtree") {
      ...
      for (const [name, item] of before) {
        if (item.entry.id === source.entry.id) {
          handles.delete(name);
          before.delete(name);
        }
      }
    ...
    if (
      ...
      if (source.state === destination.state) {
        ...
        source.state.index = index_for(source.state.file, source.state.buffer);
      } else {
        apply_compiled(source.state, compiled.source, operation_order);
        apply_compiled(
          destination.state,
      ...
      const moved_binding_names = [];
      if (operation.type === "move_subtree") {
        for (const [name, item] of before) {
          if (
            item.binding.file_id === source.state.file.id &&
            item.entry.id === source.entry.id
          ) {
            moved_binding_names.push(name);
          }
        }
      }
      const destination_binding = {
        file_id: destination.state.file.id,
        node_type: source_fingerprint.node_type,
        document: destination.entry.document,
        path: [],
        semantic_digest: source_fingerprint.semantic_digest,
      };
      const result_entry = matching_handle_entry(
        destination.state,
        destination_binding,
      );
      for (const name of moved_binding_names) {
        handles.set(name, {
          ...destination_binding,
  ...
  const transaction_proof = create_transaction_proof(
    files.map((file) => ({
      source_path: file.path || file.file_id,
      proof: file.proof,
    })),
    operation_order,
  );
