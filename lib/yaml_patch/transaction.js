...
const MAX_EVIDENCE_RAW_BYTES = 4 * 1024;
...
function participant_digest_for(files) {
  ...
  return canonical_digest(
    files
      .map((file, index) => {
        const label = `transaction file ${index}`;
        assert_plain_known(file, FILE_FIELD, label);
        assert_non_empty_string(file.id, `${label}.id`, "REQUEST_ERROR");
        if (ids.has(file.id)) {
          throw_request_error(`Duplicate transaction file id: ${file.id}`);
        }
        ids.add(file.id);
        if (file.digest !== undefined) {
          assert_sha256_digest(file.digest, `${label}.digest`, "REQUEST_ERROR");
        }
        return {
          id: file.id,
...
function validate_transaction_preconditions(request, options) {
  const canonical_profile_digest = options.profile
    ? canonical_digest(options.profile)
    : null;
  if (
    canonical_profile_digest !== null &&
    options.profile_digest !== undefined &&
    options.profile_digest !== canonical_profile_digest
  ) {
    precondition_error(
      "transaction",
      "Supplied profile digest does not match the profile object",
  ...
  const profile_digest =
    canonical_profile_digest || options.profile_digest || null;
...
function refresh_operation_target_handle(handles, snapshot, target) {
  if (target.handle === undefined) return;
  const before = snapshot.get(target.handle);
  if (!before) {
    throw new Yaml_patch_error(
      "PRECONDITION_FAILED",
      "Operation target handle vanished during refresh",
      { details: { handle: target.handle } },
    );
  }
  const refreshed = resolve_query_path(
    target.state.index,
    before.entry.document,
    before.entry.path,
  );
  if (!refreshed || refreshed.node_type !== before.binding.node_type) {
    throw new Yaml_patch_error(
      "PRECONDITION_FAILED",
      "Operation target handle no longer identifies its exact node",
      { details: { handle: target.handle } },
    );
  }
  handles.set(target.handle, {
    ...
    document: refreshed.document,
    path: structuredClone(refreshed.path),
    semantic_digest: entry_semantic_digest(target.state.index, refreshed),
  });
  snapshot.delete(target.handle);
}

function collection_entries(state, target, index_field) {
  const relationship =
    index_field === "sequence_index" ? "sequence_item" : "mapping_value";
  return target.entry.child_ids
    .map((child_id) => state.index._internal.entry_by_id.get(child_id))
    .filter((entry) => entry && entry.relationship === relationship)
    .sort((left, right) => left[index_field] - right[index_field]);
}

function mapping_pair_index_for(target, operation) {
  const pair = operation.pair;
  if (Number.isSafeInteger(pair.index)) return pair.index;
  if (typeof pair.key_raw_digest === "string") {
    const matches = target.entry.child_key_digests
      .map((digest, index) => ({ digest, index }))
      .filter(({ digest }) => digest === pair.key_raw_digest);
    return matches.length === 1 ? matches[0].index : null;
  }
  if (typeof pair.locator === "string") {
    const addressable_index = build_addressable_index(target.state.index);
    const target_addressable = addressable_index.node_entry_by_id.get(
      target.entry.id,
    );
    const addressable = addressable_index.entries.find(
      (entry) =>
        entry.addressable_type === "mapping_pair" &&
        target_addressable &&
        entry.parent_id === target_addressable.id &&
        entry.locator === pair.locator,
    );
    return addressable ? addressable.mapping_pair_index : null;
  }
  return null;
}

function sequence_position_index_for(target, position) {
  const item_count = collection_entries(
    target.state,
    target,
    "sequence_index",
  ).length;
  if (position.kind === "append") return item_count;
  if (position.kind === "prepend") return 0;
  if (position.kind === "index") return position.index;
  const item = position.item;
  const expected = item && (item.current_entry || item);
  const requested_index = Number.isSafeInteger(position.index)
    ? position.index
    : item && Number.isSafeInteger(item.index)
      ? item.index
      : null;
  let index = requested_index;
  if (index === null && expected && typeof expected.locator === "string") {
    const addressable_index = build_addressable_index(target.state.index);
    const target_addressable = addressable_index.node_entry_by_id.get(
      target.entry.id,
    );
    ...
    index = addressable ? addressable.sequence_index : null;
  }
  if (index === null) return null;
  return position.kind === "before" ? index : index + 1;
}

function reorder_permutation(target, operation, index_field) {
  const entries = collection_entries(target.state, target, index_field);
  const item_count = entries.length;
  let permutation;
  if (
    index_field === "sequence_index" &&
    operation.type === "reorder_sequence_items"
  ) {
    permutation = operation.indices;
  } else if (
    index_field === "sequence_index" &&
    operation.type === "swap_sequence_items"
  ) {
    ...
    index_field === "mapping_pair_index" &&
    operation.type === "reorder_mapping_pairs"
  ) {
    permutation = operation.pairs.map((pair) =>
      mapping_pair_index_for(target, { pair }),
    );
  } else if (
    (index_field === "sequence_index" &&
      operation.type === "move_sequence_item") ||
    (index_field === "mapping_pair_index" &&
      operation.type === "move_mapping_pair")
  ) {
    const source_index =
      index_field === "sequence_index"
        ? operation.index
        : mapping_pair_index_for(target, operation);
    const position_index =
      index_field === "sequence_index"
        ? sequence_position_index_for(target, operation.position)
        : mapping_position_index_for(target, operation.position);
    if (source_index === null || position_index === null) return null;
    permutation = Array.from({ length: item_count }, (_, index) => index);
    const [moved] = permutation.splice(source_index, 1);
    permutation.splice(
      position_index > source_index ? position_index - 1 : position_index,
      0,
      moved,
    );
  }
...
function mapping_position_index_for(target, position) {
  const item_count = collection_entries(
    target.state,
    target,
    "mapping_pair_index",
  ...
  if (position.kind === "prepend") return 0;
  if (position.kind === "index") return position.index;
  if (!position.pair) return null;
  const index = mapping_pair_index_for(target, { pair: position.pair });
  if (index === null) return null;
  return position.kind === "before" ? index : index + 1;
}

function refresh_reordered_collection_handles(
  handles,
  snapshot,
  target,
  permutation,
  ...
  if (!permutation) {
    throw new Yaml_patch_error(
      "PRECONDITION_FAILED",
      "Collection reorder position cannot preserve handle identity",
    );
  }
  const target_path_length = target.entry.path.length;
  const entries = collection_entries(target.state, target, index_field);
  for (const [name, before] of snapshot) {
    if (
      before.binding.file_id !== target.state.file.id ||
      !path_starts_with(before.entry.path, target.entry.path) ||
      before.entry.path.length <= target_path_length
    ) {
      continue;
    }
    const item_step = before.entry.path[target_path_length];
    const source_index = item_step[index_field];
    if (!Number.isSafeInteger(source_index)) continue;
    const next_index = permutation.indexOf(source_index);
    if (next_index < 0 || !entries[next_index]) continue;
    const next_path = structuredClone(before.entry.path);
    next_path[target_path_length] = {
      ...next_path[target_path_length],
      ...entries[next_index].path[target_path_length],
    };
...
function removed_collection_entries(target, operation, index_field) {
  const entries = collection_entries(target.state, target, index_field);
  if (index_field === "sequence_index") {
    if (operation.type === "delete_sequence_item") {
      return entries[operation.index] ? [entries[operation.index]] : [];
    }
    const matches = entries.filter((entry) =>
      typed_values_equal(typed_for_entry(target.state, entry), operation.value),
    );
    return operation.type === "delete_one_sequence_value"
      ? matches.slice(0, 1)
      : matches;
  }
  const pair_index = mapping_pair_index_for(target, operation);
  return pair_index === null || !entries[pair_index]
    ? []
    : [entries[pair_index]];
}

function create_structural_handle_transform(target, operation) {
  const index_field = MAPPING_OPERATION.has(operation.type)
    ? "mapping_pair_index"
    : "sequence_index";
  const insertion_types = new Set([
    "add_mapping_pair",
  ...
  if (insertion_types.has(operation.type)) {
    return { kind: "insert", index_field };
  }
  const deletion_types = new Set([
    "delete_mapping_pair",
    "delete_sequence_item",
    "delete_one_sequence_value",
    "delete_all_sequence_values",
  ]);
  if (deletion_types.has(operation.type)) {
    return {
      kind: "delete",
      index_field,
      removed_entries: removed_collection_entries(
        target,
        operation,
        index_field,
      ).sort((left, right) => right[index_field] - left[index_field]),
    };
  }
  const reorder_types = new Set([
  ...
  if (reorder_types.has(operation.type)) {
    return {
      kind: "reorder",
      index_field,
      permutation: reorder_permutation(target, operation, index_field),
    };
  }
  return null;
}

function refresh_structural_collection_handles(
  handles,
  snapshot,
  target,
  transform,
  ...
  if (!transform) return;
  if (transform.kind === "insert" && compiled.result_range) {
    const result_entry = entry_for_result_range(
      target.state,
      compiled.result_range,
    );
    refresh_destination_insertion_handles(
      handles,
      snapshot,
      target,
      result_entry,
  ...
  if (transform.kind === "delete") {
    for (const entry of transform.removed_entries) {
      refresh_deleted_subtree_handles(handles, snapshot, {
        state: target.state,
        entry,
      });
    }
  ...
  if (transform.kind === "reorder") {
    const current_entry = resolve_query_path(
      target.state.index,
      target.entry.document,
      target.entry.path,
    );
    if (!current_entry || current_entry.node_type !== target.entry.node_type) {
      throw new Yaml_patch_error(
        "PRECONDITION_FAILED",
        "Edited collection no longer identifies its declared node",
      );
    }
    refresh_reordered_collection_handles(
      handles,
      snapshot,
      { ...target, entry: current_entry },
      transform.permutation,
      transform.index_field,
    );
  }
}
...
function indexed_path_step(step) {
  if (Number.isSafeInteger(step.sequence_index)) {
    return { field: "sequence_index", index: step.sequence_index };
  }
  if (Number.isSafeInteger(step.mapping_pair_index)) {
    return { field: "mapping_pair_index", index: step.mapping_pair_index };
  }
  return null;
}

function path_after_child_removal(path, removed_path) {
  const parent_path = removed_path.slice(0, -1);
  if (
    !path_starts_with(path, parent_path) ||
    path.length <= parent_path.length
  ) {
    return structuredClone(path);
  }
  const removed = indexed_path_step(removed_path[removed_path.length - 1]);
  const sibling = indexed_path_step(path[parent_path.length]);
  if (!removed || !sibling || removed.field !== sibling.field) {
    return structuredClone(path);
  }
  const next_path = structuredClone(path);
  if (sibling.index > removed.index) {
    next_path[parent_path.length] = {
      ...next_path[parent_path.length],
      [sibling.field]: sibling.index - 1,
    };
  }
  return next_path;
}

function path_after_child_insertion(path, inserted_path) {
  const parent_path = inserted_path.slice(0, -1);
  if (
    !path_starts_with(path, parent_path) ||
    path.length <= parent_path.length
  ) {
  ...
  const inserted = indexed_path_step(inserted_path[inserted_path.length - 1]);
  const sibling = indexed_path_step(path[parent_path.length]);
  if (!inserted || !sibling || inserted.field !== sibling.field) {
    return structuredClone(path);
  }
  const next_path = structuredClone(path);
  if (sibling.index >= inserted.index) {
    next_path[parent_path.length] = {
      ...next_path[parent_path.length],
      [sibling.field]: sibling.index + 1,
    };
...
function refresh_destination_insertion_handles(
  handles,
  snapshot,
  destination,
  result_entry,
  ...
  for (const [name, before] of snapshot) {
    if (before.binding.file_id !== destination.state.file.id) continue;
    const next_path = path_after_child_insertion(
      before.entry.path,
      result_entry.path,
    );
    if (canonical_digest(next_path) === canonical_digest(before.entry.path)) {
      continue;
    }
    const next_entry = resolve_query_path(
    ...
    if (
      !next_entry ||
      next_entry.node_type !== before.binding.node_type ||
      entry_semantic_digest(destination.state.index, next_entry) !==
        before.binding.semantic_digest
    ) {
      throw new Yaml_patch_error(
        "PRECONDITION_FAILED",
        "Inserted subtree shifted a handle away from its declared node",
        { details: { handle: name } },
      );
    }
    handles.set(name, {
...
function refresh_relocated_subtree_handles(
  handles,
  states,
  snapshot,
  source,
  ...
  for (const [name, before] of snapshot) {
    let next_path = structuredClone(before.entry.path);
    if (before.binding.file_id === source.state.file.id) {
      next_path = path_after_child_removal(next_path, source.entry.path);
    }
    if (before.binding.file_id === destination.state.file.id) {
      next_path = path_after_child_insertion(next_path, result_entry.path);
    }
    const state = states.get(before.binding.file_id);
    const next_entry = resolve_query_path(
      state.index,
      before.entry.document,
      next_path,
    );
    if (!next_entry || next_entry.node_type !== before.binding.node_type) {
      throw new Yaml_patch_error(
        "PRECONDITION_FAILED",
        "Relocated subtree handle no longer identifies its declared node",
        { details: { handle: name } },
      );
    }
    handles.set(name, {
      ...
      document: next_entry.document,
      path: structuredClone(next_entry.path),
      semantic_digest: entry_semantic_digest(state.index, next_entry),
    });
  }
}
...
function typed_for_entry(state, entry, raw = raw_for_entry(state, entry)) {
  if (entry.node_type !== "scalar") return null;
  const node = get_index_node(state.index, entry);
  if (!node) return null;
  const metadata = typed_scalar_metadata(node, raw);
...
function bounded_raw_evidence(
  raw_buffer,
  raw_digest = sha256_digest(raw_buffer),
) {
  const truncated = raw_buffer.length > MAX_EVIDENCE_RAW_BYTES;
  let raw = raw_buffer
    .subarray(0, truncated ? MAX_EVIDENCE_RAW_BYTES : raw_buffer.length)
    .toString("utf8");
  while (Buffer.byteLength(raw) > MAX_EVIDENCE_RAW_BYTES) {
    raw = raw.slice(0, -1);
  }
  return {
    raw,
...
function evidence_for_entry(state, entry) {
  const raw_buffer = state.buffer.subarray(
    entry.source.start_byte,
    entry.source.end_byte,
  );
  const bounded = bounded_raw_evidence(raw_buffer, entry.raw_digest);
  return {
    ...bounded,
    typed: bounded.truncated
      ? null
      : typed_for_entry(state, entry, bounded.raw),
  };
}
...
function evidence_for_target(operation, resolved) {
  return {
    ...
    before: evidence_for_entry(resolved.state, resolved.entry),
  };
}
...
function validate_operation_preconditions(
  ...
  if (preconditions.raw !== undefined) {
    const actual_raw = raw_for_entry(state, entry);
    if (preconditions.raw !== actual_raw) {
      const actual_buffer = state.buffer.subarray(
        entry.source.start_byte,
        entry.source.end_byte,
      );
      precondition_error(
        "operation",
        "Operation raw precondition failed",
        bounded_raw_evidence(Buffer.from(preconditions.raw)),
        bounded_raw_evidence(actual_buffer, entry.raw_digest),
        { operation_id: operation.id, field: "raw" },
      );
    }
  }
  if (preconditions.typed !== undefined) {
  ...
  for (const [operation_order, operation] of request.operations.entries()) {
    ...
    if (operation.type === "bind") {
      ...
      operation_evidence.push({
        ...
        before: evidence_for_entry(state, entry),
        after: evidence_for_entry(state, entry),
        no_op: true,
      });
      continue;
    }
    if (operation.type === "add_subtree") {
      ...
      operation_evidence.push({
        ...
        before: null,
        after: evidence_for_entry(destination.state, result_entry),
        no_op: false,
      });
      continue;
    }
    ...
    if (
      ...
      const source_before = evidence_for_entry(source.state, source.entry);
      ...
      if (operation.type === "move_subtree") {
        refresh_relocated_subtree_handles(
          handles,
          states,
          before,
          source,
    ...
    const handle_transform = create_structural_handle_transform(
      target,
      operation,
    );
    apply_compiled(target.state, compiled, operation_order);
    refresh_operation_target_handle(handles, before, target);
    refresh_structural_collection_handles(
      handles,
      before,
      target,
      handle_transform,
    ...
    operation_evidence.push({
      ...
      after: candidate_entry
        ? evidence_for_entry(target.state, candidate_entry)
        : null,
