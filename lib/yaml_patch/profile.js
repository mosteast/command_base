...
const GRAPH_FIELDS = Object.freeze(["rule_id", "reference_rules", "acyclic"]);
...
function validate_reference_rule(rule, index, node_sets, identity_by_id) {
  const label = `reference rule ${index}`;
  assert_known_fields(rule, REFERENCE_FIELDS, label, "VALIDATION_FAILED");
  for (const field of [
    "rule_id",
    "source_node_set",
    "source_field",
  ...
  if (!Object.hasOwn(node_sets, rule.source_node_set)) {
    profile_error(`${label} references unknown source node set`);
  }
  if (Object.hasOwn(rule, "source_identity")) {
    assert_non_empty_string(
      rule.source_identity,
      `${label}.source_identity`,
      "VALIDATION_FAILED",
    );
    const source_identity = identity_by_id.get(rule.source_identity);
    if (!source_identity)
      profile_error(`${label} references unknown source identity`);
    if (source_identity.node_set !== rule.source_node_set) {
      profile_error(`${label} source identity belongs to a different node set`);
    }
  }
  if (!Object.hasOwn(node_sets, rule.target_node_set)) {
    profile_error(`${label} references unknown target node set`);
  }
  const target_identity = identity_by_id.get(rule.target_identity);
  if (!target_identity) profile_error(`${label} references unknown identity`);
  if (target_identity.node_set !== rule.target_node_set) {
    profile_error(`${label} target identity belongs to a different node set`);
  }
  const source_path = identity_field_path(
    rule.source_field,
    `${label}.source_field`,
  );
  const source_fields = node_sets[rule.source_node_set].fields;
  if (
    source_fields &&
    !(source_fields.allowed || []).includes(source_path[0])
  ) {
    profile_error(`${label}.source_field must be declared in fields.allowed`);
  }
  assert_known_fields(
    rule.cardinality,
    ["kind", "min", "max"],
  ...
  if (!new Set(["scalar", "sequence"]).has(rule.cardinality.kind)) {
    profile_error(`${label}.cardinality.kind must be scalar or sequence`);
  }
  validate_cardinality(
    { min: rule.cardinality.min, max: rule.cardinality.max },
    `${label}.cardinality`,
  );
  if (!NULL_MISSING_POLICY_SET.has(rule.null_policy)) {
    profile_error(`${label}.null_policy is unsupported`);
  }
  if (!IDENTITY_SCOPE_SET.has(rule.resolution_scope)) {
    profile_error(`${label}.resolution_scope is unsupported`);
  }
  assert_boolean(rule.unique_values, `${label}.unique_values`);
}

function validate_graph_rule(rule, index, reference_by_id) {
  const label = `graph rule ${index}`;
  assert_known_fields(rule, GRAPH_FIELDS, label, "VALIDATION_FAILED");
  for (const field of ["rule_id", "reference_rules", "acyclic"]) {
    if (!Object.hasOwn(rule, field))
      profile_error(`${label} requires ${field}`);
  }
  assert_non_empty_string(
  ...
  const reference_rules = string_list(
    rule.reference_rules,
    `${label}.reference_rules`,
    { non_empty: true },
  );
  for (const reference_rule of reference_rules) {
    if (!reference_by_id.has(reference_rule)) {
      profile_error(
        `${label} references unknown reference rule: ${reference_rule}`,
      );
    }
    if (rule.acyclic && !reference_by_id.get(reference_rule).source_identity) {
      profile_error(
        `${label} requires source_identity on every acyclic reference rule`,
      );
    }
  }
  assert_boolean(rule.acyclic, `${label}.acyclic`);
}
...
function diagnostic_rule_ids(profile) {
  ...
  for (const rules of [
    ...
    for (const rule of rules) rule_ids.add(rule.rule_id);
  }
  for (const [node_set_name, node_set] of Object.entries(
    profile.node_sets || {},
  )) {
    const fields = node_set.fields;
    if (!fields) continue;
    rule_ids.add(`${node_set_name}.fields.allowed`);
    for (const field of fields.required || []) {
      rule_ids.add(`${node_set_name}.fields.${field}.required`);
    }
    for (const [field, rule] of Object.entries(fields.rules || {})) {
      if (rule.cardinality) {
        rule_ids.add(`${node_set_name}.fields.${field}.cardinality`);
      }
      if (rule.types) rule_ids.add(`${node_set_name}.fields.${field}.type`);
      if (rule.consistent_type) {
        rule_ids.add(`${node_set_name}.fields.${field}.consistent_type`);
      }
      if (rule.child_node_set) {
        rule_ids.add(`${node_set_name}.fields.${field}.child_node_set`);
      }
    }
  }
  return rule_ids;
...
function validate_per_operation_rule(profile) {
  const selected_rules = Object.hasOwn(profile, "per_operation_rule")
    ? profile.per_operation_rule
    : [];
  const known_rule_ids = diagnostic_rule_ids(profile);
  for (const rule_id of string_list(
    selected_rules,
    "profile.per_operation_rule",
  )) {
    if (!known_rule_ids.has(rule_id)) {
      profile_error(
        `profile.per_operation_rule references an unknown rule_id: ${rule_id}`,
        { rule_id },
      );
    }
  }
...
function validate_profile(profile) {
  ...
  const identity_by_id = new Map(identity.map((rule) => [rule.rule_id, rule]));
  const references = profile.references || [];
  assert_array(references, "profile references");
  references.forEach((rule, index) =>
    validate_reference_rule(rule, index, node_sets, identity_by_id),
  );
  const reference_by_id = new Map(
    references.map((rule) => [rule.rule_id, rule]),
  );
  const graphs = profile.graphs || [];
  assert_array(graphs, "profile graphs");
  graphs.forEach((rule, index) =>
    validate_graph_rule(rule, index, reference_by_id),
  );
  validate_cross_references(profile);
  validate_unique_rule_ids(profile);
  validate_per_operation_rule(profile);
