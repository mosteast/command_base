...
function position_index(context, target, layout, position, key) {
  if (position === undefined) {
    const order = context.profile && context.profile.suggested_mapping_order;
...
function compile_set(context, target, operation, layout) {
  ...
  if (!value || !Array.isArray(value.range) || !YAML.isScalar(value)) {
    unsupported_shape(
      "set_mapping_value supports only existing scalar values",
      target,
    );
  }
