...
function pair_index_for(context, target, layout, reference, label = "pair") {
  ...
  if (fields[0] === "index") {
    if (!Number.isSafeInteger(reference.index)) {
      request_error(`${label} index must be a safe integer`);
    }
  ...
  if (fields[0] === "key_raw_digest") {
    if (typeof reference.key_raw_digest !== "string") {
      request_error(`${label} key_raw_digest must be a string`);
    }
  ...
  if (typeof reference.locator !== "string" || reference.locator.length === 0) {
    request_error(`${label} locator must be a non-empty string`);
  }
...
function profile_field_order(context, target) {
  ...
  for (const [name, node_set] of matches.slice(1)) {
    if (
      ...
      request_error("Matching profile node sets have conflicting field_order", {
...
function position_index(context, target, layout, position, key) {
  ...
  if (position.kind === "prepend" || position.kind === "append") {
    if (Object.keys(position).length !== 1) {
      request_error(`Mapping ${position.kind} position only accepts kind`);
    }
  ...
  if (position.kind === "index") {
    if (
      ...
      request_error("Mapping index position requires only an integer index");
    }
  ...
  if (["before", "after"].includes(position.kind)) {
    if (
      ...
      request_error(`Mapping ${position.kind} position requires only pair`);
    }
...
function subtree_pair_buffer(context, pair, value, layout) {
  ...
  const value_prefix = Buffer.from(
    `:${layout.line_break}${child_indent}`,
    "utf8",
  );
  const buffer = Buffer.concat([key_buffer, value_prefix, serialized_value]);
  const value_start_offset = key_buffer.length + value_prefix.length;
  return {
    buffer,
    value_start_offset,
