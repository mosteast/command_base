...
async function execute_migration_request(request, options = {}) {
  const {
    kind: _kind,
    command: _command,
    action,
  ...
  const sources = await load_source_buffers(migration_request.files, options);
  const runtime = { ...options, sources };
  if (action) {
    const plan = options.plan || plan_migration(migration_request, runtime);
    const result = await resume_migration(plan, {
      ...runtime,
      action,
      completed_batch_ids,
      batch_id,
      files: migration_request.files,
      request: migration_request,
      write: options.write,
    });
    return success_response({ kind: "migration", ...result });
  }
  const plan = plan_migration(migration_request, runtime);
  if (options.write && plan.batches[0]) {
    const batch = await execute_migration_batch(plan, plan.batches[0], {
      ...runtime,
      files: migration_request.files,
      write: true,
    });
    return success_response({ kind: "migration", plan, batch });
  }
  return success_response({ kind: "migration", ...plan });
}
...
async function execute_request(request, options = {}) {
  try {
    ...
    if (kind === "migration" || Array.isArray(request.rules)) {
      return execute_migration_request(request, options);
    }
    if (
      kind === "transaction" ||
