...
function validate_operation_binding(binding, label, root_range) {
  assert_proof_object(
    binding,
    new Set([
      "operation_id",
  ...
  if (
    !Array.isArray(binding.original_spans) ||
    !Array.isArray(binding.candidate_spans)
  ) {
    byte_error(`${label} spans must be arrays`);
  }
  let previous_original_end = -1;
  for (const [index, span] of binding.original_spans.entries()) {
    const span_label = `${label}.original_spans[${index}]`;
    assert_proof_object(span, new Set(["start_byte", "end_byte"]), span_label);
    assert_proof_integer(span.start_byte, `${span_label}.start_byte`);
    assert_proof_integer(span.end_byte, `${span_label}.end_byte`);
    if (
      span.end_byte < span.start_byte ||
      span.start_byte < previous_original_end ||
      (root_range &&
        (span.start_byte < root_range.original_start_byte ||
          span.end_byte > root_range.original_end_byte))
    ) {
      byte_error(`${span_label} is noncanonical or escapes its shared root`);
    }
    previous_original_end = span.end_byte;
  }
  let previous_candidate_end = -1;
  for (const [index, span] of binding.candidate_spans.entries()) {
    const span_label = `${label}.candidate_spans[${index}]`;
    assert_proof_object(span, new Set(["start_byte", "end_byte"]), span_label);
    assert_proof_integer(span.start_byte, `${span_label}.start_byte`);
    assert_proof_integer(span.end_byte, `${span_label}.end_byte`);
    if (
      span.end_byte < span.start_byte ||
      span.start_byte < previous_candidate_end ||
      (root_range &&
        (span.start_byte < root_range.candidate_start_byte ||
          span.end_byte > root_range.candidate_end_byte))
    ) {
      byte_error(`${span_label} is noncanonical or escapes its shared root`);
    }
    previous_candidate_end = span.end_byte;
  }
  assert_proof_digest(binding.removed_digest, `${label}.removed_digest`);
  assert_proof_digest(
    binding.replacement_digest,
    `${label}.replacement_digest`,
  );
  return binding;
}
...
function validate_proof_span(value, label, digest_fields = []) {
  ...
  if (value.operation_bindings !== undefined) {
    if (!Array.isArray(value.operation_bindings)) {
      byte_error(`${label}.operation_bindings must be an array`);
    }
    for (const [index, binding] of value.operation_bindings.entries()) {
      validate_operation_binding(
        binding,
        `${label}.operation_bindings[${index}]`,
        value,
      );
    }
  }
...
function validate_proof_operation(operation, index) {
  ...
  operation.final_ranges.forEach((range, range_index) => {
    ...
    if (
      ...
      range.operation_bindings !== undefined ||
...
function removed_bytes_from_ranges(original_buffer, removed_ranges) {
  if (!Array.isArray(removed_ranges) || removed_ranges.length === 0) {
    return Buffer.alloc(0);
  }
  return Buffer.concat(
    removed_ranges.map((range) =>
      original_buffer.subarray(range.start_byte, range.end_byte),
    ),
  );
}

function bytes_from_spans(buffer, spans) {
  if (!Array.isArray(spans) || spans.length === 0) return Buffer.alloc(0);
  return Buffer.concat(
    spans.map((span) => buffer.subarray(span.start_byte, span.end_byte)),
  );
}

function binding_matches_reconstructible_buffers(
  binding,
  step,
  original_buffer,
  candidate_buffer,
  ...
  if (
    step.removed_digest !== binding.removed_digest ||
    step.replacement_digest !== binding.replacement_digest
  ) {
    return false;
  }
  if (step.removed_original_ranges.length > 0) {
    const removed = removed_bytes_from_ranges(
      original_buffer,
      step.removed_original_ranges,
    );
    if (sha256_digest(removed) !== binding.removed_digest) return false;
  }
  if (binding.candidate_spans.length === 0) return true;
  return (
    sha256_digest(
      bytes_from_spans(candidate_buffer, binding.candidate_spans),
    ) === binding.replacement_digest
  );
}
...
function validate_reconstructible_step_digests(
  ...
  for (const [index, range] of ranges.entries()) {
    if (!Array.isArray(range.operation_bindings)) continue;
    for (const [binding_index, binding] of range.operation_bindings.entries()) {
      const operation = operations_by_id.get(binding.operation_id);
      const step = operation && operation.steps[0];
      if (
        !step ||
        !binding_matches_reconstructible_buffers(
          binding,
          step,
          original_buffer,
          candidate_buffer,
        ...
        byte_error(
          `Byte proof range ${index} binding ${binding_index} does not bind the supplied buffers`,
        );
      }
    }
  }
  for (const [index, operation] of operations.entries()) {
    ...
    if (
      named_ranges.length === 1 &&
      Array.isArray(named_ranges[0].operation_bindings)
    ) {
      const binding = named_ranges[0].operation_bindings.find(
...
function validate_named_step_digests(operations, ranges) {
  ...
  for (const range of ranges) {
    ...
    for (const [binding_index, binding] of range.operation_bindings.entries()) {
      ...
      if (
        ...
        byte_error(
          `byte proof range binding ${binding_index} does not name a single-step final operation`,
        );
      }
      const step = operation.steps[0];
      if (
        ...
        byte_error(
          `Byte proof operation ${operation.operation_id} step digests do not bind its shared root`,
        );
      }
    }
  }
  for (const operation of operations) {
    ...
    if (named_ranges.length !== 1) continue;
    const root_range = named_ranges[0];
    if (root_range.operation_ids.length !== 1) continue;
    const final_range = operation.final_ranges[0];
    const step = operation.steps[0];
    if (!proof_range_coordinates_equal(final_range, root_range)) continue;
    ...
    if (!removed_range_matches) continue;
    if (
...
function validate_byte_proof(proof, options = {}) {
  ...
  for (const [index, range] of validated.ranges.entries()) {
    ...
    if (validated.version === 2) {
      ...
      if (range.operation_ids.length === 1) {
        if (range.operation_bindings !== undefined) {
          byte_error(
            `byte proof range ${index} has operation bindings on an exclusive root`,
          );
        }
      } else if (
        !Array.isArray(range.operation_bindings) ||
        range.operation_bindings.length !== range.operation_ids.length ||
        range.operation_bindings.some(
          (binding, binding_index) =>
            binding.operation_id !== range.operation_ids[binding_index],
        )
      ) {
        byte_error(
          `byte proof range ${index} is missing verifiable shared-root operation bindings`,
        );
      }
    } else if (range.operation_ids !== undefined) {
      byte_error(`Version 1 byte proof range ${index} has operation IDs`);
    } else if (range.operation_bindings !== undefined) {
      byte_error(`Version 1 byte proof range ${index} has operation bindings`);
    }
...
function candidate_spans_for_operation(table, range, operation_id) {
  const spans = [];
  let candidate_cursor = 0;
  for (const piece of table.pieces) {
    const length = piece_length(piece);
    const piece_end = candidate_cursor + length;
    if (
      piece.kind === "insert" &&
      piece.operation_ids.includes(operation_id) &&
      piece_end > range.candidate_start_byte &&
      candidate_cursor < range.candidate_end_byte
    ) {
      const overlap_start = Math.max(
        candidate_cursor,
        range.candidate_start_byte,
      );
      const overlap_end = Math.min(piece_end, range.candidate_end_byte);
      if (overlap_end > overlap_start) {
        const previous = spans[spans.length - 1];
        if (previous && previous.end_byte === overlap_start) {
          previous.end_byte = overlap_end;
        } else {
          spans.push({ start_byte: overlap_start, end_byte: overlap_end });
        }
      }
    }
    candidate_cursor = piece_end;
...
function original_spans_for_edit(edit, range) {
  return (edit.removed_original_ranges || [])
    .filter(
      (removed_range) =>
        removed_range.end_byte > range.original_start_byte &&
        removed_range.start_byte < range.original_end_byte,
    )
    .map((removed_range) => ({
      start_byte: Math.max(removed_range.start_byte, range.original_start_byte),
      end_byte: Math.min(removed_range.end_byte, range.original_end_byte),
    }))
    .filter((span) => span.end_byte > span.start_byte);
}

function create_operation_bindings(
  table,
  range,
  operations_by_id,
  candidate_buffer,
) {
  return range.operation_ids.map((operation_id) => {
    const operation = operations_by_id.get(operation_id);
    if (!operation || operation.steps.length !== 1) {
      byte_error("Shared-root operation lacks a single reconstructible step", {
        operation_id,
      });
    }
    const step = operation.steps[0];
    const edit = table.edit_log.find(
      (entry) => entry.operation_id === operation_id,
    );
    if (!edit) {
      byte_error("Shared-root operation is missing from the piece table", {
        operation_id,
      });
    }
    const candidate_spans = candidate_spans_for_operation(
      table,
      range,
      operation_id,
    );
    const attributed_digest = sha256_digest(
      bytes_from_spans(candidate_buffer, candidate_spans),
    );
    return {
      operation_id,
      original_spans: original_spans_for_edit(edit, range),
      // Only keep final candidate spans when they still equal the
      // application-time replacement. Later shared-root edits may carve the
      // replacement into a proper subset that cannot rebind that digest.
      candidate_spans:
        attributed_digest === step.replacement_digest ? candidate_spans : [],
      removed_digest: step.removed_digest,
      replacement_digest: step.replacement_digest,
    };
  });
}
...
function create_multi_range_byte_proof(
  ...
  for (const range of ranges) {
    if (range.operation_ids.length > 1) {
      range.operation_bindings = create_operation_bindings(
        table,
        range,
        operations_by_id,
        candidate_buffer,
