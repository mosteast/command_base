...
function apply_validation_scope(inputs, scope) {
  if (
  ...
  const files = new Set(scope.files.map((file) => path.resolve(file)));
  return inputs.filter((input) => files.has(input.source_path));
}
...
function diagnostic(record, input) {
  ...
  return create_diagnostic({
    ...
    ...(node_set && (node_set.diagnostic_projection || []).length > 0
      ? { projection: projection_for(record, node_set) }
      : {}),
  });
}
...
function validate_identity_limits(changes, limits = {}) {
  ...
  for (const field of Object.keys(limits)) {
    if (!IDENTITY_LIMIT_FIELDS.includes(field)) {
...
function sort_diagnostics(diagnostics) {
  return diagnostics.sort((left, right) => {
    ...
    return canonical_json(left.path) < canonical_json(right.path) ? -1 : 1;
  });
}
...
function validate_profile_candidates(input) {
  ...
  const original_inputs = apply_validation_scope(
    apply_profile_path_scope(original_all, profile, options),
    normalized_scope,
  );
  const candidate_inputs = apply_validation_scope(
    apply_profile_path_scope(candidate_all, profile, options),
    normalized_scope,
  );
  const original_node_sets = build_node_sets(
    original_inputs,
    profile,
    max_result,
  );
  const candidate_node_sets = build_node_sets(
    candidate_inputs,
    profile,
    max_result,
  );
