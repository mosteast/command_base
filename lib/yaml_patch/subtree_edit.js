...
const SUBTREE_OPERATION_FIELD = Object.freeze({
  add_subtree: ["id", "type", "raw", "key", "position"],
  copy_subtree: ["id", "type", "position"],
  delete_subtree: ["id", "type"],
  move_subtree: ["id", "type", "position"],
});
...
function validate_document_dependencies(context, target, side) {
  const document = context.index.parser_result.documents[target.document];
  const unresolved_tags = context.index.parser_result.warnings.filter(
    (warning) =>
      warning.document === target.document &&
      warning.code === "TAG_RESOLVE_FAILED",
  );
  if (unresolved_tags.length > 0) {
    cross_boundary(`${side} document contains an unresolved tag handle`, {
      document: target.document,
      warning_count: unresolved_tags.length,
    });
  }
  visit_yaml(document && document.contents, (node) => {
    ...
    let anchor;
    try {
      anchor = node.resolve(document);
    } catch (error) {
      cross_boundary(`${side} document contains an unresolvable alias`, {
        alias: node.source,
      });
    }
    if (!anchor) {
      cross_boundary(`${side} document contains a dangling alias`, {
        alias: node.source,
      });
    }
  });
...
function subtree_dependency(context, target) {
  ...
  validate_document_dependencies(context, target, "Source");
...
function assert_destination_anchor_safety(
  ...
  validate_document_dependencies(
    destination_context,
    destination_target,
    "Destination",
  );
...
function replace_nested_record_range(records, splice) {
  const containing_records = records.filter(
    (record) =>
      splice.start_byte >= record.start_byte &&
      splice.end_byte <= record.end_byte,
  );
  if (containing_records.length !== 1) {
    cross_boundary(
      "Nested subtree removal cannot be contained in one destination item",
      { containing_record_count: containing_records.length },
    );
  }
  const containing_record = containing_records[0];
  const relative_start = splice.start_byte - containing_record.start_byte;
  const relative_end = splice.end_byte - containing_record.start_byte;
  return records.map((record) =>
    record === containing_record
      ? {
          ...record,
          buffer: Buffer.concat([
            record.buffer.subarray(0, relative_start),
            splice.replacement_buffer,
            record.buffer.subarray(relative_end),
          ]),
        }
      : record,
  );
...
function compile_subtree_operation(
  ...
  assert_known_fields(
    operation,
    ["id", "type", "raw", "key", "position"],
    "Subtree operation",
    "REQUEST_ERROR",
  );
  if (!SUBTREE_OPERATION.has(operation.type)) {
    throw_request_error("Unsupported subtree operation");
  }
  assert_known_fields(
    operation,
    SUBTREE_OPERATION_FIELD[operation.type],
  ...
  if (
    operation.type === "move_subtree" &&
    source_context.index === destination_context.index &&
    dependency.subtree_nodes.has(
      get_index_node(destination_context.index, destination_target),
    )
  ) {
    cross_boundary("A subtree cannot be moved into its own descendant", {
      source_locator: source_target.locator,
      destination_locator: destination_target.locator,
    });
  }
  ...
  if (
    operation.type === "move_subtree" &&
    source_context.index === destination_context.index &&
    source_target.parent_id === destination_target.id
  ) {
    const reordered_records = source_removal_records(
      source_layout,
      source_record,
      source_index,
      source_target,
    );
    const reordered_result_index =
      result_index > source_index ? result_index - 1 : result_index;
    reordered_records.splice(
      reordered_result_index,
      0,
      rebase_record(
        source_record,
  ...
  if (
    operation.type === "move_subtree" &&
    source_context.index === destination_context.index &&
    source_layout.start_byte > destination_layout.start_byte &&
    source_layout.end_byte <= destination_layout.end_byte
  ) {
    const source_result = replacement_result(
      operation,
      source_layout,
      source_removal_records(
        source_layout,
    ...
    const combined_records = replace_nested_record_range(destination_records, {
      ...source_result.splices[0],
      start_byte: Math.min(
        source_result.splices[0].start_byte,
        source_record.start_byte,
      ),
    });
    combined_records.splice(
      result_index,
      0,
      rebase_record(
        source_record,
