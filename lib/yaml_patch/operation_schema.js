...
function require_field(operation, field) {
  if (!Object.hasOwn(operation, field)) {
    request_error(`${operation.type} requires ${field}`);
  }
}

function assert_index(value, label) {
  if (!Number.isSafeInteger(value)) {
    request_error(`${label} must be a safe integer`);
  }
}

function validate_operation(operation, allowed_types, label) {
  if (!operation || typeof operation !== "object" || Array.isArray(operation)) {
    request_error(`${label} operation must be an object`);
  }
  ...
  if (typeof operation.id !== "string" || operation.id.length === 0) {
    request_error(`${label} operation id must be a non-empty string`);
  }
  ...
  if (
    ...
    request_error(`${operation.type} style must be a non-empty string`);
  }
  if (
    operation.type === "replace_scalar_raw" &&
    typeof operation.raw !== "string"
  ) {
    request_error("replace_scalar_raw raw must be a string");
  }
  if (
    ["add_mapping_pair", "rename_mapping_key"].includes(operation.type) &&
    typeof operation.key !== "string"
  ) {
    request_error(`${operation.type} key must be a string`);
  }
  ...
  if (operation.type === "reorder_sequence_items") {
    if (!Array.isArray(operation.indices)) {
      request_error("reorder_sequence_items indices must be an array");
    }
  ...
  if (
    ...
    request_error("reorder_mapping_pairs pairs must be an array");
  }
  return operation;
}
