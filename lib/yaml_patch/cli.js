...
const { execute_request } = require("./request");
...
function build_help_text() {
  return [
    ...
    `  ${SCRIPT_NAME} query --query <query.json|-> [--json]`,
    ...
    `  ${SCRIPT_NAME} transaction --operations <tx.json|-> [--profile <file>] [--write] [--json]`,
    `  ${SCRIPT_NAME} migration --operations <migration.json|-> [--write] [--json]`,
    `  ${SCRIPT_NAME} status --journal <journal> [--json]`,
    `  ${SCRIPT_NAME} recover --journal <journal> --direction <commit|rollback> [--json]`,
    `  ${SCRIPT_NAME} replay --manifest <manifest.json> [--json]`,
    ...
    "  the selected range. Apply, patch, and transaction are dry-run unless --write",
    "  is explicit. Multi-file writes use a durable journal rather than simultaneous",
    "  visibility.",
    "",
    chalk.bold("Commands"),
    "  inspect, find, query, extract, apply, patch, transaction, migration, status,",
    "  recover, replay, validate, capabilities, and lock management commands.",
    "",
    chalk.bold("Options"),
    "  --query <file>                 Version 1/2 JSON query; use - for stdin",
    "  --operations <file>            Version 1/2 JSON operation or transaction document",
    "                                 Use - to read one bounded JSON document from stdin",
    "  --profile <file>               Declarative validation profile",
    "  --manifest <file>              Transaction manifest for replay",
    "  --journal <file>               Durable transaction journal path",
    "  --direction <value>            Recovery direction: commit or rollback",
    ...
    "  --refresh                       Replace generated same-kind artifacts (default: false)",
    ...
    "",
    "  # Run a composable query v2 document from stdin",
    `  $0 query --query - --json < query_v2.json`,
    "",
    "  # Dry-run a multi-file transaction",
    `  $0 transaction --operations tx.json --profile profile.yaml --json`,
    "",
    "  # Write a transaction and inspect recovery status",
    `  $0 transaction --operations tx.json --write --json`,
    `  $0 status --journal .yaml_patch-transaction-*.journal --json`,
    `  $0 recover --journal journal.json --direction rollback --json`,
    "",
    "  # Replay a saved manifest against current inputs",
    `  $0 replay --manifest manifest.json --json`,
    "",
...
function create_argument_parser(args) {
  return yargs(args)
    ...
    .option("profile", { type: "string", requiresArg: true })
    .option("manifest", { type: "string", requiresArg: true })
    .option("journal", { type: "string", requiresArg: true })
    .option("direction", {
      type: "string",
...
function create_stdin_guard(logger) {
  let stdin_consumed = false;
  return {
    async read(source, label) {
      return read_request_input(source, {
        label,
        stdin: logger.stdin,
        max_bytes: DEFAULT_JSON_INPUT_BYTES,
        get stdin_consumed() {
          return stdin_consumed;
        },
        mark_stdin_consumed() {
          stdin_consumed = true;
        },
...
async function run_query_command(argv, logger) {
  if (!argv.query) throw request_error("query requires --query");
  logger.debug("stage: execute query request");
  logger.debug(`io: read query ${argv.query}`);
  const stdin = create_stdin_guard(logger);
  const query = await stdin.read(argv.query, "query");
  const response = await execute_request(
    { kind: "query", ...query },
    {
      max_file_bytes: argv["max-file-byte"],
      debug: argv.debug ? (message) => logger.debug(message) : undefined,
    },
  );
  if (response.ok === false) {
    throw new Yaml_patch_error(response.code, response.message, {
...
async function run_transaction_command(argv, logger) {
  if (!argv.operations) {
    throw request_error("transaction requires --operations");
  }
  logger.debug("stage: execute transaction request");
  logger.debug(`io: read operations ${argv.operations}`);
  const stdin = create_stdin_guard(logger);
  const operations_document = await stdin.read(argv.operations, "operations");
  let profile;
  if (argv.profile) {
    logger.debug(`io: read profile ${argv.profile}`);
    profile = await stdin.read(argv.profile, "profile");
  }
  const response = await execute_request(
    {
      kind: "transaction",
    ...
    {
      write: Boolean(argv.write) && !argv["dry-run"],
      max_file_bytes: argv["max-file-byte"],
...
async function run_migration_command(argv, logger) {
  if (!argv.operations) {
    throw request_error("migration requires --operations");
  }
  logger.debug("stage: execute migration request");
  const stdin = create_stdin_guard(logger);
  const document = await stdin.read(argv.operations, "operations");
  const response = await execute_request(
...
async function run_status_command(argv, logger) {
  if (!argv.journal) throw request_error("status requires --journal");
  logger.debug(`io: inspect journal ${argv.journal}`);
  const response = await execute_request({
    kind: "status",
    journal_path: path.resolve(argv.journal),
  });
  if (response.ok === false) {
    throw new Yaml_patch_error(response.code, response.message, {
      details: response.details,
...
async function run_recover_command(argv, logger) {
  if (!argv.journal) throw request_error("recover requires --journal");
  if (!argv.direction) {
    throw request_error("recover requires --direction commit|rollback");
  }
  logger.debug(
    `stage: recover journal ${argv.journal} direction=${argv.direction}`,
  );
  const response = await execute_request({
    kind: "recover",
    journal_path: path.resolve(argv.journal),
...
async function run_replay_command(argv, logger) {
  if (!argv.manifest) throw request_error("replay requires --manifest");
  logger.debug(`io: read manifest ${argv.manifest}`);
  const stdin = create_stdin_guard(logger);
  const manifest = await stdin.read(argv.manifest, "manifest");
  const response = await execute_request(
...
async function dispatch_command(command, patterns, argv, logger) {
  ...
  if (command === "query") return run_query_command(argv, logger);
  ...
  if (command === "transaction") return run_transaction_command(argv, logger);
  if (command === "migration") return run_migration_command(argv, logger);
  if (command === "status") return run_status_command(argv, logger);
  if (command === "recover") return run_recover_command(argv, logger);
  if (command === "replay") return run_replay_command(argv, logger);
