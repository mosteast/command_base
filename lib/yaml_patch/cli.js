...
function build_help_text() {
  return [
    ...
    "",
    chalk.bold("Options"),
    "  --query <file>                 Version 1 JSON query",
    ...
    `  $0 extract config.yaml --query query.json --edit-unit scalar-token --output .yamlpatch/session`,
    "",
    "  # Preview an edited fragment without writing",
    `  $0 apply .yamlpatch/session --json`,
    "",
    "  # Atomically apply an edited fragment",
    `  $0 apply .yamlpatch/session --write --json`,
    "",
    ...
    `  $0 extract-lock-info .yamlpatch/session --json`,
    `  $0 break-stale-extract-lock .yamlpatch/session --lock-token <token> --json`,
  ].join("\n");
}
...
async function read_json_file(file_path, logger, label) {
  ...
  try {
    const file = await read_bounded_file(resolved_path, {
      ...
      file_type_error_code: "VALIDATION_FAILED",
      limit_error_code: "VALIDATION_FAILED",
    });
    ...
    throw new Yaml_patch_error(
      "VALIDATION_FAILED",
      `Cannot read ${label} JSON: ${resolved_path}`,
      { cause: error, details: { path: resolved_path } },
    );
  }
}
...
async function run_find(patterns, argv, logger) {
  ...
  if (!Number.isInteger(result_limit) || result_limit <= 0) {
    throw new Yaml_patch_error(
      "VALIDATION_FAILED",
      "--max-result must be a positive integer",
    );
  }
  ...
  if (!Number.isInteger(max_output_bytes) || max_output_bytes <= 0) {
    throw new Yaml_patch_error(
      "VALIDATION_FAILED",
      "--max-output-byte must be a positive integer",
    );
  }
  const matches = [];
...
async function run_cli(args = process.argv.slice(2), io = process) {
  ...
  try {
    ...
    if (!COMMANDS.has(command_value)) {
      throw new Yaml_patch_error(
        "VALIDATION_FAILED",
        `Unknown command: ${command_value || "<none>"}`,
      );
    }
    ...
    io.stdout.write(serialize_response(error_response(error)));
    return 1;
  }
}
