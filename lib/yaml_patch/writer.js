...
async function apply_edit_package(edit_package, options = {}) {
  ...
  debug_log(
    options,
    `stage: probe no-op before acquiring write lock ${source_path}`,
  );
  const probe = await compile_edit_package(edit_package, options);
  if (probe.patch_result.no_op) {
    debug_log(
      options,
      `stage: no-op write skips lock and artifacts ${source_path}`,
    );
    return {
      ...public_patch_result(probe.patch_result),
      written: false,
      dry_run: false,
    };
  }
