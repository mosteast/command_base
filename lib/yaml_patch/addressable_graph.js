...
function validate_node_index_integrity(index) {
  if (
    ...
    throw precondition_error(
      "Addressable graph requires a complete node index",
    );
  }
  const actual_source_digest = sha256_digest(index.source.buffer);
  if (actual_source_digest !== index.source.digest) {
    throw validation_error("Source bytes changed after v1 indexing", {
      expected_source_digest: index.source.digest,
  ...
  if (
    ...
    throw precondition_error("Node-index maps and entries disagree", {
      entry_count: index.entries.length,
      entry_map_size: index._internal.entry_by_id.size,
      node_map_size: index._internal.node_by_id.size,
    });
  }
  const seen_ids = new Set();
  index.entries.forEach((entry, ordinal) => {
    const node = entry && index._internal.node_by_id.get(entry.id);
    if (
      !entry ||
      !Number.isInteger(entry.id) ||
      seen_ids.has(entry.id) ||
      ...
      throw precondition_error("Node-index entry identity is inconsistent", {
        ordinal,
  ...
  if (index.document_root_ids.length !== index.parser_result.documents.length) {
    throw precondition_error("Document roots and parsed documents disagree");
  }
  index.document_root_ids.forEach((root_id, document_index) => {
    const root_entry = index._internal.entry_by_id.get(root_id);
    const root_node = index._internal.node_by_id.get(root_id);
    if (
      !root_entry ||
      root_entry.document !== document_index ||
      root_entry.relationship !== "document_value" ||
      root_node !== index.parser_result.documents[document_index].contents
    ) {
      throw precondition_error("Document root identity is inconsistent", {
