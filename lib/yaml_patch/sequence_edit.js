...
const { clone_json_operation_value } = require("./json_value");
...
function sequence_item_index_for(layout, reference) {
  if (!reference || typeof reference !== "object" || Array.isArray(reference)) {
    throw_request_error("sequence position item must identify a current item");
  }
  if (Object.hasOwn(reference, "index")) {
    ...
    if (!Number.isSafeInteger(reference.index)) {
      throw_request_error(
        "sequence position item index must be a safe integer",
      );
    }
  ...
  if (
    ...
    throw_request_error("sequence position item locator accepts only locator");
  }
  if (
    reference.current_entry !== undefined &&
...
function position_index(layout, position) {
  if (!position || typeof position !== "object" || Array.isArray(position)) {
    throw_request_error("Sequence insertion requires a position object");
  }
  if (position.kind === "prepend" || position.kind === "append") {
    if (Object.keys(position).length !== 1) {
      throw_request_error(
        `Sequence ${position.kind} position only accepts kind`,
      );
    }
  ...
  if (position.kind === "index") {
    if (
      ...
      throw_request_error(
        "Sequence index position requires only an integer index",
      );
    }
  ...
  if (position.kind === "before" || position.kind === "after") {
    if (position.item === undefined && !Object.hasOwn(position, "index")) {
      throw_request_error(
        "sequence before/after position requires item or index",
      );
    }
    if (
      Object.keys(position).length !== 2 ||
  ...
  throw_request_error(`Unsupported sequence position: ${position.kind}`);
}
...
function compile_insert(context, target, operation, layout) {
  if (!Object.hasOwn(operation, "value"))
    throw_request_error("Sequence insertion requires value");
  let position;
  ...
  const safe_value = clone_json_operation_value(
    operation.value,
    `${operation.type} value`,
  );
  const records = layout.records.slice();
  records.splice(position, 0, {
    buffer: new_item_buffer(safe_value, layout.indent, layout.line_break),
  });
...
function compile_cross_file_move(
  ...
  if (operation.type !== "move_sequence_item") {
    throw_request_error("Cross-file sequence move requires move_sequence_item");
  }
...
function compile_operation(context, target, operation) {
  ...
  throw_request_error(`Unsupported sequence operation: ${operation.type}`);
}
