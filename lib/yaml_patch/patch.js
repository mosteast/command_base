...
function prepare_operation_patch(index, patch_document) {
  ...
  if (
    patch_document.version !== 1 ||
    !Array.isArray(patch_document.operations)
  ) {
    throw new Yaml_patch_error(
      "VALIDATION_FAILED",
  ...
  assert_known_fields(
    operation_entry,
    new Set(["target", "operation", "limits"]),
    "patch operation entry",
    validation_code,
  );
  assert_known_fields(
    operation_entry.target,
    new Set(["locator", "expected_digest"]),
    "patch target",
    validation_code,
  );
  assert_known_fields(
  ...
  const selected_entry = entry_from_locator(
    index,
    operation_entry.target && operation_entry.target.locator,
  );
  if (!selected_entry) {
    throw new Yaml_patch_error(
      "NO_MATCH",
  ...
  if (
    operation_entry.target.expected_digest &&
    operation_entry.target.expected_digest !== selected_entry.raw_digest
  ) {
  ...
  if (operation.type === "set_mapping_value") {
    ...
    if (
      !YAML.isMap(selected_node) ||
      typeof operation.key !== "string" ||
      !Object.hasOwn(operation, "value")
    ) {
      throw new Yaml_patch_error(
        "INVALID_FRAGMENT",
        "set_mapping_value requires a mapping target and string key",
      );
    }
    ...
    if (
      ...
      throw new Yaml_patch_error(
        "VALIDATION_FAILED",
        "replace_scalar_token requires a raw string value",
      );
    }
    ...
    if (
      ...
      throw new Yaml_patch_error(
        "VALIDATION_FAILED",
        "replace_node_value requires a raw string value",
      );
    }
