...
function clone_operation_value(value) {
  try {
    return clone_json_value(value, "set_mapping_value value");
  } catch (error) {
    throw request_error("set_mapping_value value must be JSON-safe data", {
      cause: error,
...
function serialize_cloned_operation_value(cloned_value) {
  return Buffer.from(YAML.stringify(cloned_value).replace(/\n$/, ""), "utf8");
}

function serialize_operation_value(value) {
  return serialize_cloned_operation_value(clone_operation_value(value));
}
...
function validate_operation_request_entry(operation_entry) {
  ...
  let validated_operation = operation;
  if (operation.type === "set_mapping_value") {
    ...
    validated_operation = {
      type: operation.type,
      key: operation.key,
      value: clone_operation_value(operation.value),
    };
  } else if (
    !Object.hasOwn(operation, "value") ||
  ...
  return { limits, operation: validated_operation, target };
}
...
function prepare_operation_patch(index, patch_document) {
  ...
  if (operation.type === "set_mapping_value") {
    ...
    replacement_buffer = serialize_cloned_operation_value(operation.value);
