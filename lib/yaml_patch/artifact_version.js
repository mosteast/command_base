...
function clone_limit_error(limit_name, limit, actual, path) {
  return new Yaml_patch_error(
    "CHANGE_LIMIT_EXCEEDED",
    `JSON clone exceeds ${limit_name}`,
    { details: { limit_name, limit, actual, path } },
  );
}

function clone_limit_option(options, name) {
...
function consume_clone_node(state, path, depth) {
  if (depth > state.max_depth) {
    throw clone_limit_error("max_depth", state.max_depth, depth, path);
  }
  state.nodes += 1;
  if (state.nodes > state.max_nodes) {
    throw clone_limit_error("max_nodes", state.max_nodes, state.nodes, path);
  }
}

function assert_clone_child_capacity(state, count, path) {
  if (count > state.max_nodes - state.nodes) {
    throw clone_limit_error(
      "max_nodes",
      state.max_nodes,
      state.nodes + count,
      path,
...
function clone_json_value_internal(value, path, ancestors, state, depth) {
  consume_clone_node(state, path, depth);
  if (
    value === null ||
    typeof value === "string" ||
    typeof value === "boolean"
  ...
  try {
    if (Array.isArray(value)) {
      const length_descriptor = Object.getOwnPropertyDescriptor(
        value,
        "length",
      );
      const array_length = length_descriptor && length_descriptor.value;
      if (array_length > state.max_array_items) {
        throw clone_limit_error(
          "max_array_items",
          state.max_array_items,
          array_length,
          path,
        );
      }
      const values = canonical_array_values(value, path);
      assert_clone_child_capacity(state, values.length, path);
      return values.map((entry, index) =>
        clone_json_value_internal(
          entry,
          `${path}[${index}]`,
          ancestors,
          state,
    ...
    assert_clone_child_capacity(state, properties.length, path);
    const cloned = {};
    for (const property of properties) {
      ...
      Object.defineProperty(cloned, property, {
        value: clone_json_value_internal(
          descriptor.value,
          property_path,
          ancestors,
          state,
...
function clone_json_value(value, label = "$", options = {}) {
  ...
  try {
    if (!options || typeof options !== "object" || Array.isArray(options)) {
      throw canonicalization_error(
        "$",
        options,
        "clone options must be an object",
      );
    }
    return clone_json_value_internal(
      value,
      path,
      new WeakSet(),
      {
        nodes: 0,
        max_nodes: clone_limit_option(options, "max_nodes"),
        max_depth: clone_limit_option(options, "max_depth"),
        max_array_items: clone_limit_option(options, "max_array_items"),
      },
      0,
    );
  } catch (error) {
