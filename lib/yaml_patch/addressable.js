...
function first_meaningful_token_range(source, tokens, kind) {
  ...
  for (const token of tokens || []) {
    if (!ignored_types.has(token.type)) {
      return token_character_range(source, token, kind);
    }
  }
  return null;
}

function mapping_pair_character_range(source, mapping_node, pair, pair_index) {
  ...
  if (
    ...
    collection_items[pair_index] !== pair.srcToken
  ) {
  ...
  const end_range = pair.srcToken.value
  ...
  if (!start_range || !end_range) {
    throw validation_error("Cannot derive mapping pair range from CST", {
      pair_index,
    });
  }

  const start_character = start_range.start_character;
  const end_character = end_range.end_character;
  ...
  for (const node of [pair.key, pair.value]) {
    if (
      ...
      (node.range[0] < start_character || node.range[1] > end_character)
    ) {
      throw validation_error("Mapping pair does not contain its YAML node", {
        pair_index,
        pair_range: [start_character, end_character],
...
function sequence_item_character_range(
  ...
  const indicator_range = first_meaningful_token_range(
    source,
    item_token.start,
    "sequence item start",
  );
  const value_range = item_token.value
    ? token_character_range(source, item_token.value, "sequence item value")
    : null;
  const start_range = indicator_range || value_range;
  const end_range =
    value_range ||
    (Array.isArray(item_token.start) && item_token.start.length > 0
      ? token_character_range(source, item_token.start, "sequence item")
      : start_range);
  if (!start_range || !end_range) {
    throw validation_error("Cannot derive sequence item range from CST", {
      sequence_index,
    });
  }
  ...
  if (
    ...
    (item_node.range[0] < start_character || item_node.range[1] > end_character)
  ) {
    throw validation_error("Sequence item does not contain its YAML node", {
      sequence_index,
      item_range: [start_character, end_character],
...
function typed_scalar_metadata(node, raw) {
  ...
  if (node.tag) {
    ...
    return scalar_type ? { scalar_type, scalar_value: node.value } : {};
  }
  if (node.value === null) {
...
function build_addressable_index(index, options = {}) {
  ...
  function add_node(v1_entry, parent_id) {
    ...
    if (YAML.isMap(node)) {
      node.items.forEach((pair, pair_index) => {
        ...
        const pair_path = clone_path(
          value_v1_entry ? value_v1_entry.path : key_v1_entry.path,
        );
      ...
      node.items.forEach((item_node, sequence_index) => {
        ...
        add_node(child_v1_entry, item_entry.id);
      });
    }
    return node_entry;
  }
